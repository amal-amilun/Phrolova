...
CONFIG = {
    ...
    "trailing_stop_atr_multiplier": 3,
}
...
def trend_following_strategy(df, current_position):
    ...
    if(prev_data['MA_fast'] < prev_data['MA_slow'] and latest_data['MA_fast'] > latest_data['MA_slow'] and not current_position):
        action = 'buy'
        ...
        try:
            ...
            if position_data['in_position']:
                if manage_trailing_stop(latest_data['close'], position_data):
                    coin_balance = balances.get(CONFIG['coin'], 0)
                    if coin_balance > 0:
                        print(f"Menjual seluruh {coin_balance} {CONFIG['coin']}")
                        position_data['in_position'] = False
                        exchange.create_market_sell_order(CONFIG['symbol'], coin_balance)
            ...
            if action == "buy":
                ...
                if usdt_balance > 10:
                    print(f"Membeli {CONFIG['coin']} dengan seluruh {usdt_balance} USDT..")
